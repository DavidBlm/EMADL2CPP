...
import os
import mxnet as mx
from mxnet import gluon, nd, random
...
class SACTrainingStats(TrainingStats):
    def __init__(self, max_episodes):
        super(SACTrainingStats, self).__init__(max_episodes)
        self._all_avg_critic_loss = np.zeros((max_episodes,))
        self._all_avg_actor_loss = np.zeros((max_episodes,))
        self._all_avg_qvalues = np.zeros((max_episodes,))
        self._all_avg_logprob = np.zeros((max_episodes,))
        self._all_avg_temperature = np.zeros((max_episodes,))
        self._all_avg_temperature_loss = np.zeros((max_episodes,))
    ...
    def add_avg_logprob(self, episode, avg_logprob):
        self._all_avg_logprob[episode] = avg_logprob

    def add_avg_temperature(self, episode, avg_temperature):
        self._all_avg_temperature[episode] = avg_temperature
    
    def add_avg_temperature_loss(self, episode, avg_temperature_loss):
        self._all_avg_temperature_loss[episode] = avg_temperature_loss

    def _log_episode(self, episode, start_time, training_steps, reward):
        self.add_total_reward(episode, reward)
        end = time.time()
        mean_reward_last_100 = self.mean_of_reward(episode, last=100)
        time_elapsed = end - start_time
        ...
        return ('Episode: %d, Total Reward: %.3f, '
                'Avg. Reward Last 100 Episodes: %.3f, {}, '
                'Time: %.3f, Training Steps: %d') % (
                    episode, reward, mean_reward_last_100, time_elapsed,
                    training_steps), mean_reward_last_100

    def log_episode(
        self, episode, start_time, training_steps, actor_loss,
        critic_loss, qvalues, logprob, temperature, temperature_loss, reward
    ):
        self.add_avg_actor_loss(episode, actor_loss)
        self.add_avg_critic_loss(episode, critic_loss)
        self.add_avg_qvalues(episode, qvalues)
        self.add_avg_logprob(episode, logprob)
        self.add_avg_temperature(episode, temperature)
        self.add_avg_temperature_loss(episode, temperature_loss)

        info, avg_reward = self._log_episode(
            episode, start_time, training_steps, reward)
        info = info.format((
            'Avg. Actor Loss: %.3f '
            'Avg. Critic Loss: %.3f '
            'Avg. Q-Values: %.3f '
            'Avg. LogProb: %.3f '
            'Avg. Temperature: %.3f '
            'Avg. Temperature-Loss: %.3f ') % (actor_loss, critic_loss, qvalues, logprob, temperature, temperature_loss))
    ...
    def save(self, path, episode=None):
        if episode is None:
            episode = self._max_episodes
        super(SACTrainingStats, self).save(path, episode=episode)
        np.save(os.path.join(
            path, 'avg_critic_loss'), self._all_avg_critic_loss[:episode])
        np.save(os.path.join(path, 'avg_actor_loss'), self._all_avg_actor_loss[:episode])
        np.save(os.path.join(path, 'avg_qvalues'), self._all_avg_qvalues[:episode])
        np.save(os.path.join(path, 'avg_temperature'), self._all_avg_temperature[:episode])
        np.save(os.path.join(path, 'avg_temperature_loss'), self._all_avg_temperature_loss[:episode])
        np.save(os.path.join(path, 'avg_logprob'), self._all_avg_logprob[:episode])
        np.save(os.path.join(path, 'mean_reward_last_100_episodes'), self._all_avg_logprob[:episode])
    ...
    def save_stats(self, path, episode=None):
        ...
        all_avg_critic_loss = self._all_avg_critic_loss[:episode]
        all_avg_qvalues = self._all_avg_qvalues[:episode]
        all_avg_temperature = self._all_avg_temperature[:episode]
        all_avg_temperature_loss = self._all_avg_temperature_loss[:episode]
        all_avg_logprob = self._all_avg_logprob[:episode]
        all_mean_reward_last_100_episodes = self._all_mean_reward_last_100_episodes[:episode]

        fig = plt.figure(figsize=(40, 40))

        sub_rewards = fig.add_subplot(331)
        sub_rewards.set_title('Total Rewards per episode')
        sub_rewards.plot(
            np.arange(episode), all_total_rewards)

        sub_actor_loss = fig.add_subplot(332)
        sub_actor_loss.set_title('Avg. Actor Loss per episode')
        sub_actor_loss.plot(
            np.arange(episode), all_avg_actor_loss)

        sub_critic_loss = fig.add_subplot(333)
        sub_critic_loss.set_title('Avg. Critic Loss per episode')
        sub_critic_loss.plot(
            np.arange(episode), all_avg_critic_loss)

        sub_qvalues = fig.add_subplot(334)
        sub_qvalues.set_title('Avg. QValues per episode')
        sub_qvalues.plot(
            np.arange(episode), all_avg_qvalues)

        sub_temperature = fig.add_subplot(335)
        sub_temperature.set_title('Avg. Temperature per episode')
        sub_temperature.plot(np.arange(episode), all_avg_temperature)

        sub_temperature_loss = fig.add_subplot(336)
        sub_temperature_loss.set_title('Avg. Temperature Loss per episode')
        sub_temperature_loss.plot(np.arange(episode), all_avg_temperature_loss)

        sub_logprob = fig.add_subplot(337)
        sub_logprob.set_title('Avg. Log probability per episode')
        sub_logprob.plot(np.arange(episode), all_avg_logprob)

        sub_rewards = fig.add_subplot(338)
        sub_rewards.set_title('Avg. mean reward of last 100 episodes')
        sub_rewards.plot(np.arange(episode),
                         all_mean_reward_last_100_episodes)
